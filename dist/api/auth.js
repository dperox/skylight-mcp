...
function parseAuthenticityToken(html) {
...
function parseAuthorizationCode(location, expectedState) {
...
async function beginOAuthFlow(state, codeChallenge, cookieJar) {
    const response = await fetchWithCookies(buildAuthorizeUrl({ state, codeChallenge, prompt: "login" }), {
        method: "GET",
        headers: {
...
async function loadLoginForm(loginUrl, cookieJar) {
...
async function submitLoginForm(email, password, authenticityToken, cookieJar) {
    const body = new URLSearchParams({
        authenticity_token: authenticityToken,
        email,
        password,
    });
    const response = await fetchWithCookies(`${SKYLIGHT_BASE_URL}/auth/session`, {
        method: "POST",
        headers: {
            Accept: "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8",
...
async function authorizeAuthenticatedSession(authorizeUrl, cookieJar) {
    const response = await fetchWithCookies(authorizeUrl, {
        method: "GET",
        headers: {
            Accept: "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8",
            Referer: `${SKYLIGHT_BASE_URL}/auth/session/new`,
...
async function detectSubscriptionStatus(token) {
    try {
