...
export function registerChoreTools(server) {
    // get_chores tool
    server.tool("get_chores", `Get chores from Skylight.
...
Returns chores with their assignees, due dates, and completion status.`, {
    ...
    // create_chore tool
    server.tool("create_chore", `Add a new chore to Skylight.
...
The chore will appear on the Skylight display.`, {
    ...
    // update_chore tool
    server.tool("update_chore", `Update an existing chore in Skylight.
...
Returns: The updated chore details.`, {
        ...
        try {
            ...
            if (applyToSeries) {
                ...
                if (summary !== undefined)
                    templateUpdates.summary = summary;
                if (rewardPoints !== undefined)
                    templateUpdates.reward_points = rewardPoints;
                if (categoryId !== undefined)
                    templateUpdates.category_id = categoryId;
            ...
            if (date !== undefined)
                updates.start = parseDate(date, config.timezone);
            if (time !== undefined)
                updates.startTime = time ? parseTime(time) : null;
            if (rewardPoints !== undefined)
    ...
    // delete_chore tool
    server.tool("delete_chore", `Delete a chore from Skylight.
