...
export function registerFamilyTools(server) {
    // get_family_members tool
    server.tool("get_family_members", `Get family members/profiles from Skylight.
...
- "Who can I assign chores to?"`, {}, async () => {
        try {
    ...
    // get_frame_info tool
    server.tool("get_frame_info", `Get Skylight household/frame information.

Useful for setup verification and debugging.

Use this to answer:
- "Show Skylight household info"
- "What's my frame ID?"`, {}, async () => {
        try {
    ...
    // get_devices tool
    server.tool("get_devices", `List Skylight devices in the household.

Use this to answer:
- "What Skylight devices do we have?"
- "How many Skylight frames are connected?"`, {}, async () => {
        try {
