import { z } from "zod";
import { getRewards, getRewardPoints, addRewardPoints, createReward, updateReward, deleteReward, redeemReward, unredeemReward, } from "../api/endpoints/rewards.js";
...
- "Who has the most points?"`, {}, async () => {
        try {
            ...
            const categoryMap = new Map(categories.map((c) => [Number(c.id), c.attributes.label ?? "Unknown"]));
...
Returns: The redeemed reward details.`, {
    ...
    server.tool("add_reward_points", `Add or subtract reward points for a family member.
