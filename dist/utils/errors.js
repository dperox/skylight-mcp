...
export class SkylightError extends Error {
    ...
    statusCode;
    recoverable;
    constructor(message, code, statusCode, recoverable = false) {
        super(message);
        this.code = code;
...
export class AuthenticationError extends SkylightError {
    constructor(message = "Authentication failed. Your token may be expired or invalid.") {
...
export class RateLimitError extends SkylightError {
    constructor(retryAfter) {
        super(`Rate limited by Skylight API. ${retryAfter ? `Retry after ${retryAfter}s` : "Please wait and try again."}`, "RATE_LIMITED", 429, true);
...
export class ParseError extends SkylightError {
    constructor(message = "Unexpected API response format") {
